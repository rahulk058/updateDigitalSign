...
const {escapePath,createDirectory, checkExtension, checkVariables} = require('../utilits/filemanager');
...
module.exports = {
    ...
    async folderTree(req, res, next) {
        const { path } = req.body;
        const paths = dirTree(coreFolder + createDirectory(path), {normalizePath: true, removePath:coreFolder, withChildren: true});
        console.log("1")
        res.status(200).send(paths);
    },
    async folderInfo(req, res, next) {
        const { path,user_name } = req.body;
