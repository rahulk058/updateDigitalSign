...
class ImageMapEditor extends Component {
	...
	render() {
		...
		const action = (
			<React.Fragment>
				...
				<CommonButton
					...
					shape="circle"
					icon="image"
					tooltipTitle="Download As A Image"
					onClick={onSaveImage}
					tooltipPlacement="bottomRight"
				/>
			</React.Fragment>
		...
		const titleContent = (
			<>
				<span ><Button style={{width:"100px",height:"58px",backgroundColor:"transparent",outline: "none",border: "none"}} onClick={this.closeEditor}><i class="fas fa-sign-out-alt" style={{fontSize:"35px",color:"#fff"}}></i></Button></span>
				{/* <CloseIcon /> */}
			</>
		);
		const title = <Title title={titleContent} action={action} />;
		const content = (
