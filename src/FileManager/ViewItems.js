...
import useStyles from '../FileManager/Elements/Styles';
...
function ViewItems(props){
    ...
    console.log(config.icons.folder)
    const getThumb = (item) => {
        try {
            if(showImages === 'thumbs' && config.imageFiles.includes(item.extension)){
              return `${mainconfig.serverPath}${item.path}`;
            } else {
              return typeof config.icons[item.extension] !== 'undefined' ? config.icons[item.extension] : config.icons.broken
            }
        } catch (error) {
                ...
                <Droppable droppableId={item.id} type="CONTAINERITEM" isCombineEnabled>
                    {(provided, snapshot) => (
                        <>
                          ...
                          <TableCell className={classes.tableCell}><img style={{"width":"20px"}} src={config.icons.folder} /></TableCell>
